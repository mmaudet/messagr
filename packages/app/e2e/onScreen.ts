 ...
 *
 * # It was deleted for an hour, and why it came back matters
 *
 * The sender's line moved to the trust screen, so this had no user left and
 * went with it. Then the bench showed why the line belongs on the
 * conversation after all: its room holds three people -- both suites'
 * entrants and the inviter, "distinct people in the same room, which is what
 * they are" -- and a conversation with three people is not a conversation
 * with somebody, so every message names its sender.
 *
