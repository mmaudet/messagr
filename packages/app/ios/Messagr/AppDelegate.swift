...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  // APPLE'S ANSWER, AND IT ONLY EVER COMES HERE.
  //
  // iOS calls these two on the application delegate and on nothing else. They used to be
  // installed by `GULAppDelegateSwizzler`, which react-native-firebase asks for
  // (`RNFBMessaging+AppDelegate.m`, `proxyOriginalDelegateIncludingAPNSMethods`); with the pods
  // gone, nothing installs them and they are written here. #334.
  //
  // THE FAILURE THIS SHAPE AVOIDS, which is the likeliest one to write by accident: a method of
  // the right name on a class that is not the application delegate, or with a signature that
  // does not match. iOS then simply never calls it, says nothing at all, and `pushDevice.ts`
  // reports "Apple has not answered with a token yet" for ever -- true-sounding, and false.
  //
  // Both hand over to `MessagrApplePush`, which holds the answer for whenever JavaScript gets
  // round to asking. Apple regularly answers before the bundle has finished starting, and a
  // token nobody kept is a token nobody has.

  func application(
    _ application: UIApplication,
    didRegisterForRemoteNotificationsWithDeviceToken deviceToken: Data
  ) {
    MessagrApplePush.appleAnswered(deviceToken)
  }
  ...
  func application(
    ...
    MessagrApplePush.appleRefused()
    // The one line anybody debugging this has from the device itself. A registration error is
    // about this build's entitlement or Apple's availability -- "no valid aps-environment
    // entitlement string found" is the usual one -- and names no account, device or token.
    NSLog("MESSAGR_APNS_REFUSED %@", error.localizedDescription)
  }

  // Le lien ouvert pendant que l'application tourne (schéma messagr://).
  func application(
    _ app: UIApplication,
    open url: URL,
  ...
  // Le lien universel https ouvert pendant que l'application tourne.
  func application(
    _ application: UIApplication,
