...
>
> **"For everyone" is offered only on your own messages.** Redacting somebody else's is a moderation power, not a delete button. That the room defaults granted it to whoever created the conversation was unintended and is tracked as #196.

> **Amended 10 September 2026, on measurement (#196).** The asymmetry is closed in the room and not only on the screen. A conversation is created with `redact` above every level anybody in it holds, so the homeserver refuses a redaction of the other person's event **from either side** — measured, `403 M_FORBIDDEN` both ways — while Matrix's own rule that anybody may redact their own events survives untouched. Two people, each able to withdraw their own words and neither able to touch the other's, which is what the glossary already said a direct conversation was.
>
> **This costs a room version, and the choice is deliberate.** In room version 12 the creator's power is implicit and infinite: `users` comes back empty and no finite `redact` can be placed above it. Tried against a real conversation on the bench — `redact` set to 101, accepted, and the creator still redacted the other person's message with a `200`. Version 11, where the creator is an ordinary member at 100, answers `403` in both directions. What version 12 buys is a creator nobody can strip of power, which protects a room from being taken over; between two people who are supposed to be equals there is no hierarchy to protect, and the equality is worth more. Conversations created before this stay version 12 and keep the asymmetry, because nothing short of a room upgrade changes a version and an upgrade is a visible, disruptive act.
