...
`m.no_olm` — ce qu'il a raison de faire.
...
>
> `getOneTimeKeys` signs a `OneTimeKey` while its `IsSigned` flag is still
> false. `OneTimeKey.MarshalJSON` returns the bare key string in that
> state and the full object once the flag is true, so the signature covers
> `"AAAA…"` while what reaches `/keys/upload` is
> `{"key":"AAAA…","signatures":{…}}`.
>
> Any client that verifies a signed one-time key therefore rejects it,
> cannot establish an Olm session, and answers `m.room_key.withheld` with
> `m.no_olm`. Device keys are unaffected — `DeviceKeys` carries no such
> flag — which is why this only shows on the one-time key path.
>
> The fix sets the flag at construction, before signing. `SignJSON`
> already deletes `signatures` and `unsigned` before canonicalising, so
> the signed bytes become exactly what a verifier recomputes.
>
> **Reproducing.** Log in with this library, upload one-time keys, then
> claim one from any client that verifies signatures (matrix-nio,
> matrix-sdk-crypto, Element). The claim succeeds and the verification
> fails. A homeserver cannot delete a one-time key, so a batch uploaded
> before the fix stays claimable until it is exhausted.
